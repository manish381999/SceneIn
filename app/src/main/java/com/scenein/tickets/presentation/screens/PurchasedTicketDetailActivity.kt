            ...
            MaterialAlertDialogBuilder(this)
                ...
                .setPositiveButton("Reveal Now") { _, _ ->
                    ticket?.transactionId?.toIntOrNull()?.let {
                        viewModel.revealTicket(it)
                    }
                }.show()
        }
        binding.btnConfirmEntry.setOnClickListener {
            ticket?.transactionId?.toIntOrNull()?.let {
                viewModel.completeTransaction(it)
            }
        }
    ...
    private fun showDisputeDialog() {
        ...
        MaterialAlertDialogBuilder(this)
            ...
            .setPositiveButton("Submit Report") { _, _ ->
                ...
                if (reason.length > 10) {
                    ticket?.transactionId?.toIntOrNull()?.let { viewModel.createDispute(it, reason) }
                } else {
    ...
    private fun showResellDialog() {
        ...
        MaterialAlertDialogBuilder(this)
            ...
            .setPositiveButton("List for Resale") { _, _ ->
                ...
                ticket?.transactionId?.toIntOrNull()?.let { viewModel.relistTicket(it, newSellingPrice.toString()) }
            }.show()
    }
