...
import com.scenein.databinding.ActivityUserProfileBinding
...
import com.scenein.profile.data.models.StatItem
import com.scenein.profile.persentation.adapter.InterestAdapter
import com.scenein.profile.persentation.adapter.UserProfileViewPagerAdapter
import com.scenein.profile.persentation.view_model.ProfileViewModel
import com.scenein.settings.persentation.screen.SettingsActivity
...
class UserProfileActivity : AppCompatActivity() {
    ...
    private var hasConnectionStatusChanged = false
    ...
    private fun initComponents() {
        ...
        binding.ivSetting.isVisible = isViewingOwnProfile
        ...
        if (isViewingOwnProfile) {
            ...
            profileViewModel.checkConnectionStatus(profileId = targetUserId)
        }
    }
    ...
    private fun observeViewModels() {
        ...
        profileViewModel.connectionActionState.observe(this) { state ->
            if (state is NetworkState.Success) {
                hasConnectionStatusChanged = true
    ...
    private fun onClickListener() {
        ...
        binding.btnConnect.setOnClickListener {
            ...
            when (currentConnectionStatus?.connectionStatus) {
                ...
                "pending" -> { if (currentConnectionStatus?.requestSentBy == "me") { profileViewModel.removeConnection( pId) } }
    ...
    private fun showDisconnectBottomSheet() {
        ...
        bottomSheet.onDisconnectConfirmed = {
            profileUserId?.let { profileViewModel.removeConnection(it) }
        }
    ...
    private fun finishWithResult() {
        // Only send a result if the status actually changed
        if (hasConnectionStatusChanged && profileUserId != null) {
            val resultIntent = Intent().apply {
                putExtra("user_id_result", profileUserId)
                // Pass back the latest known status
                putExtra("connection_status_result", currentConnectionStatus?.connectionStatus)
                putExtra("request_sent_by_result", currentConnectionStatus?.requestSentBy)
            }
